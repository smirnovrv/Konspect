...
Процедура ПоляПриАктивизацииСтроки(Элемент)
	...
	Если ЗначениеЗаполнено(Элемент.ТекущиеДанные.Картинка) Тогда
		ТекстВставки 	= СформироватьТекстДляВставки(Элемент.ТекущиеДанные.НомерСтроки);
		ТекстСтрокиHTML = ВставкаДляСтрокиТекста(ТекстВставки);
		ТекстСтроки 	= Элемент.ТекущиеДанные.Значение;
	Иначе	
...
Функция ПолучитьТекстДляВставки_Картинки(Структура)
	...
	ТекстДляВставки = "<p></p>";
	ТекстДляВставки = ТекстДляВставки + "<img src='data:image/;base64," + Base64ДанныеФотографии + СтрокаВставкиРазмера;
