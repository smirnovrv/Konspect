...
Процедура КомандаДобавитьТекст(Команда)
	...
	Если ЗначениеЗаполнено(ТекстСтроки) Тогда
		...
		Структура.Вставить("ТекстДобавления", ТекстСтроки); 
		Структура.Вставить("НомерСтроки"	, НомерТекущейСтроки); 
		Если Не ЗначениеЗаполнено(ЭтоЗаголовок) Тогда
			Структура.Вставить("Тэг" , ПредопределенноеЗначение("Справочник.Тэги.p"));
		Иначе
			Структура.Вставить("Тэг" , ЭтоЗаголовок);
		КонецЕсли;	 
...
Процедура КомандаТаблица(Команда)
	...
	ОткрытьФорму("Справочник.Заметки.Форма.ФормаВводаТаблицы",,,,,,);
КонецПроцедуры
#КонецОбласти
...
Процедура ДобавитьСписокHTML(Структура)
	...
	Если Структура.Строки.Количество() > 0 Тогда
		...
		НоваяСтрока 		= Объект.Состав.Вставить(НомерТекущейСтроки);
		...
		Если Структура.Тэг = ПредопределенноеЗначение("Перечисление.ВидыСписковHTML.Маркированный") Тогда
			ТексДляВставки = ТексДляВставки + "</ul>";
		ИначеЕсли Структура.Тэг = ПредопределенноеЗначение("Перечисление.ВидыСписковHTML.Нумерованный") Тогда
...
Процедура ДобавитьСтрокуТаблицы(Структура)
	НомерПоследнейСтроки = ПолучитьНомерСтрокиДляВставки();
	ТаблицаСостав 			= СоставВТаблицу();
	Если НомерПоследнейСтроки > Структура.НомерСтроки Тогда
		НоваяСтрока = ТаблицаСостав.Получить(Структура.НомерСтроки - 1);
	Иначе
		НоваяСтрока = ТаблицаСостав.Вставить(Структура.НомерСтроки - 1);
	КонецЕсли;
	НоваяСтрока.Значение 	= Структура.ТекстДобавления;
	НоваяСтрока.Тэг 		= Структура.Тэг;
