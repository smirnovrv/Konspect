...
Функция СформироватьОсновнойТекст(СтруктураДляВставки)
	...
	ТабИзмСтрок = Объект.Поля.Выгрузить();
	 ...
	 Для Каждого СтрокаТЧ Из ТабИзмСтрок Цикл
...
Функция СформироватьТекстДляВставки(НомерСтроки)
	...
	Если ЗначениеЗаполнено(СтрокаТЧ.Тэг) Тогда
		...
		Если ЗначениеЗаполнено(СтрокаТЧ.Картинка) Тогда
			Структура = СформироватьСтруктуруДляВставкиФайла(СтрокаТЧ.Картинка);
			Структура.Вставить("Ширина", СтрокаТЧ.Ширина);
			Структура.Вставить("Высота", СтрокаТЧ.Высота);
		Иначе
			Структура = СформироватьСтруктуруДляВставки(СтрокаТЧ);
		КонецЕсли;
...
&НаСервере
Функция СформироватьСтруктуруДляВставки(СтрокаТЧ)
	Структура = Новый Структура;
	Структура.Вставить("Одиночный"		, СтрокаТЧ.Тэг.Одиночный);
	Структура.Вставить("Наименование"	, СтрокаТЧ.Тэг.Наименование);
...
Функция ПолучитьТекстДляВставки_Картинки(Структура)
	...
	ТекстДляВставки = "";
	ДвоичныеДанныеФотографии = Структура.Картинка.ФайлХранилище.Получить();
	Base64ДанныеФотографии = Base64Строка(ДвоичныеДанныеФотографии);

	ТекстДляВставки = "<p></p>";
	ТекстДляВставки = ТекстДляВставки + "<img src='data:image/;base64," + Base64ДанныеФотографии + "' width='350' height='450'>";

//	СтруктураСвойств = Новый Структура;
//	СтруктураСвойств.Вставить("ТекстДляВставки", ТекстДляВставки);
//	СтруктураСвойств.Вставить("Высота", Структура.Высота);
//	СтруктураСвойств.Вставить("Ширина", Структура.Ширина);	
						
	//Пока НЕ РАБОТАЕТ ограничения исходного размера
...
Функция ПолучитьТекстВставкиСРазмерами(СтруктураСвойств)
	...
	Ширина = ?(СтруктураСвойств.Ширина = 0, 150, СтруктураСвойств.Ширина);
 	...
 	//ТекстВставки = СтрЗаменить(СтруктураСвойств.ТекстДляВставки, "img", "img 'width=" + Ширина + "px height=" + Высота +"px'>");
 	
 	//ТекстВставки = СтрЗаменить(СтруктураСвойств.ТекстДляВставки, "img", "img 'width=50px height=50px '>");
 	
 	ТекстВставки = СтруктураСвойств.ТекстДляВставки;
